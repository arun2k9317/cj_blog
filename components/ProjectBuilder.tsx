...
import { Project, ContentBlock, ContentBlockType, DEFAULT_TEMPLATES } from '@/types/project';
...
export default function ProjectBuilder({ project, onSave, onImageUpload }: ProjectBuilderProps) {
  ...
  const addBlock = (type: ContentBlockType) => {
    const newBlock: ContentBlock = {
      ...
      ...(type === 'text' && { content: '', textAlign: 'left', fontSize: 'medium', fontWeight: 'normal' }),
      ...(type === 'image' && { src: '', alt: '', alignment: 'center', aspectRatio: 'auto' }),
      ...(type === 'image-gallery' && { images: [], layout: 'grid', columns: 3 }),
      ...(type === 'quote' && { text: '', alignment: 'center', style: 'minimal' }),
  ...
  const reorderBlocks = (fromIndex: number, toIndex: number) => {
    setProjectData(prev => {
      ...
      return {
        ...prev,
        contentBlocks: newBlocks.map((block, index) => ({ ...block, order: index }))
      };
    });
  };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 min-h-[600px]">
          {projectData.contentBlocks.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-96 text-gray-500">
              ...
              <h3 className="text-xl font-medium mb-2">No content blocks yet</h3>
              <p className="text-sm mb-4">Start building your project by adding content blocks</p>
              <button
